...
//
//  Apr 4 2021 1.04
//  - Bug fix in ValidateInputField() function, where validity Check failed in rare instances
//  - Bug fix in ValidateRadioButtonInputField() function, where validity Check failed in rare instances
//  - Bug fix in ValidateCheckBoxInputFields() function, where validity check failed in rare instances
//  - Added new function OutputLocalizedNumber() allowing to format numbers with commas and decimal places
//
//#endregion 
...
function ValidateInputField(id, errId) {
    ...
    if (testInputElement && typeof testInputElement.checkValidity === "function") {
...
function ValidateRadioButtonInputField(groupName, errId) {
    ...
    if (testRadioButtonElements && testRadioButtonElements.length > 0) {
        for (let idx = 0; idx < testRadioButtonElements.length; idx++) {
            ...
            if (radioButtonInput && typeof radioButtonInput.checkValidity === "function") {
...
function ValidateCheckBoxInputFields(groupName, errId) {
    ...
    if (testCheckBoxElements && testCheckBoxElements.length > 0) {
        for (let idx = 0; idx < testCheckBoxElements.length; idx++) {
            ...
            if (checkBoxInput && typeof checkBoxInput.checkValidity === "function") {
...
function RemoveClassState(id, toggleClass) {
    ...
    if (inputElement) {
        ...
        debugger;
        console.log(`Cannot find element '${id}' to remove class '${toggleClass}'`);
    }
}
//#endregion
...
function AddClassState(id, toggleClass) {
    ...
    if (inputElement) {
        ...
        debugger;
        console.log(`Cannot find element '${id}' to add class '${toggleClass}'`);
    }
}
//#endregion
...
//#region function OutputLocalizedNumber(value, decimalPlaces, locale)
// Method to convert a numeric valid to number with commas between thousands and hundreds place
// based upon passing in the value, the decimal places you want to use, and the locale the user
// wants the number output in
// If you pass only the value, it will use 2 decimal places, and the user's web browser prefered region
// see https://www.w3schools.com/jsref/jsref_tolocalestring_number.asp
function OutputLocalizedNumber(value, decimalPlaces, locale) {
    // debugger;
    // If the caller didn't specify the currency, use a default of Canadian dollars
    ...
    if (value && value.toLocaleString() !== undefined) {
        valueAsFormattedNumber = value.toLocaleString(locale, conversionRules);
    }
    return valueAsFormattedNumber;
}
//#endregion
